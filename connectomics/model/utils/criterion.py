        ...
        for i in range(self.num_target):
            ...
            for j,lopt in enumerate(self.loss_opt[i]):
                if lopt == 'WeightedMSE':
                    ...
                    out[i][j] = Py_sigmoid_focal_loss()
                elif lopt == 'Lovasz_softmax':
                    out[i][j] = Lovasz_softmax()
                elif lopt == 'OhemCrossEntropy':
                    out[i][j] = OhemCrossEntropy()
                else:
    ...
    def eval(self, pred, target, weight):
        ...
        for i in range(self.num_target):
            ...
            for j in range(len(self.loss[i])):
                if weight[i][0].shape[-1] == 1: # placeholder for no weight
                    loss += self.loss_weight[i][j]*self.loss_w[i][j]*self.loss[i][j](pred[:,cid:cid+numC], target_t)
                else:
                    loss += self.loss_weight[i][j]*self.loss_w[i][j]*self.loss[i][j](pred[:,cid:cid+numC], target_t, self.to_torch(weight[i][0]))
                    # print("{},{}. loss_weight {}, loss_w {}, self.loss[i][j] {}, cid {}, cid+numC {}".format(i,j,self.loss_weight[i][j],self.loss_w[i][j],self.loss[i][j],cid, cid+numC))
            cid += numC
